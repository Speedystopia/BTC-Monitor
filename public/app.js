...
window.BTCM_APP = (function () {
  ...
  function renderStatus() {
    ...
    $('sources').innerHTML = s.exchanges.map(e => {
      ...
      return `<span title="${e.status} ${U.escapeHtml(e.detail || '')} · last trade ${U.fmtAge(e.lastTradeAgo)} ago · book ${e.book} levels${e.bookResyncs ? ` (reloaded ${e.bookResyncs}×)` : ''}">${iconImg(e.id, 'srcic', e.name)}<i class="${cls}"></i><b class="nm">${U.escapeHtml(e.name)}</b>${share}</span>`;
